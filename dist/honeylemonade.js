/**
 * Honey Lemonade v2.6.2 (LemonadeJS v2.6.1)
...
;(function (global, factory) {
        ...
        if (a && a.length) {
            for (var i = 0; i < a.length; i++) {
                ...
                if (props == true && typeof(this[k]) !== 'undefined') {
                    o[dashToUpCase(k)] = this[k];
                } else {
                    o[dashToUpCase(k)] = a[i].value;
                }
            }
        }
        return o;
    ...
    var parse = function(element) {
        ...
        var lemonName = element.getAttribute('lemon-name');
        if (!lemonName) {
            lemonName = (/[-_]/g.test(element.tagName) || '') && element.tagName;
        }
        if (lemonName) {
            element.handler = t = getLemon(lemonName, this.components);
        }
    ...
    /**
     * Render (append) a lemonade DOM element, method or class into a root DOM element
    ...
    L.render = function(o, el, self, t, ref, ext) {
        // Root element must be a valid DOM element
        if (!isDOM(el)) {
            console.error('Not a valid DOM', el);
            return false;
        }
    ...
    L.element = function(t, self, ext) {
        ...
        if (! isDOM(t)) {
            ...
            // if no root element
            (!/^\s*<>/.test(t)) && (t = '<root>'+t+'</root>');

            // Parse fragment
            t = t.replace(/<>/gi, '<root>').replace(/<\/>/gi, '<\/root>').trim();

            // Parse Lamonade Elements
            t = t.replace(/(<([A-Z]{1}[a-zA-Z0-9_-]+)[^>]*)>/gm, function (fullTag, $1, tagName) {
                var cPath = tagName.replace(/_/g, '/');
                var cName = toLemonName(tagName);
                if (!(lemon.components[cName] || R.lemons[cName])) {
                    lemon.require.push({name: cName, path: cPath, inline: !/\/>$/.test(fullTag) });
                }
                return fullTag.replace(RegExp('<'+tagName), '<'+cName+' lemon-name="'+cName+'" lemon-path="'+cPath+'"').replace(/\/>$/, '></'+cName+'>');
            });

            importLemons(lemon.require, lemon.components, lemon.self);
    ...
    function dashToUpCase (str) {
        return str.replace(/_/g,'_-').replace(/(-.)/g,function(x){return x[1].toUpperCase()});
    }

    //component-full-name | Component-Full-Name | ComponentFullName => ComponentFullName
    function toLemonName (lemonName) {
        return dashToUpCase('-'+lemonName);
    }

    function getLemon ( n ) {
        var container = arguments[1] || R.lemons;

        if (!arguments.length) return container;

        var retVal;
        if (n && !isEmpty(container)) {
            var m = toLemonName(n);
            retVal = (typeof(container[m]) == 'function' && container[m]) || null;
        }
        return retVal;
    }

    function getSync (cPath) {
        var resTxt;

        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            var xhrStatus = xhr.status;
            if (xhr.readyState === 4 && ((xhrStatus >= 200 && xhrStatus < 300) || (xhrStatus === 304))) {
        ...
        xhr.open('GET', cPath, false);
        xhr.send();

        return resTxt;
    }

    function importLemon (cPath, parent, children, jsTmpl) {
        var cName = cPath.replace(/[/-]/g, '_');

        var jsTxt = getSync(cPath+'.js')

        if (/^</.test(jsTxt)) {
            console.warn('Failed to load component:', cPath+'.js', '\n'+jsTxt);
            jsTxt = '';
        }

        var tmplStr = '';
        if (jsTmpl && (!/(var |let |const )template\s*(=|;)/g.test(jsTxt)
        || /\/\/\s*(var |let |const )template\s*(=|;)/.test(jsTxt))) {
            tmplStr = getSync(cPath+'.html');
        }

        var jsFn = 'return function '+cName+' (inlineTmpl) {\n template = inlineTmpl || template;\n'
                +jsTxt
                +'\n;return lemonade.element(template, self, children);\n'
                +'}';

        var cFn = (new Function('template', 'path', 'parent', 'self', 'children', 'lemonade', 'get', jsFn))(tmplStr, cPath, parent, {}, children, L, getSync);

        return cFn;
    }

    // Update Global container
    function addLemons (container) {
        Object.keys(container).forEach(function(key) {
            R.lemons[key] = container[key];
        });
    }
    ...
    /** lemonade.mount(o, el, self, t, ref, ext)
     * Mount a lemonade DOM element, method or class into a root DOM element
     * @param o - Lemonade DOM created from a template
     * @param el - DOM Element (or a single element querySelector) to append the lemonade element
     * @param self - existing self
     * @param t - template when used as a custom component
     */
    function mount (o, el, self, t, ref, ext) {
        // fix o
        if (typeof(o) == 'string') {
        ...
        return L.render(o, el, self, t, ref, ext);
    }

    // export
    L.mount        = mount;
    L.importLemons = importLemons;
    L.importLemon  = importLemon;
    L.addLemons    = addLemons;
    L.getLemon     = getLemon;
